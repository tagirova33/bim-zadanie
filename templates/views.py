...
def cit(request):
    ...
    if civil.is_valid() and user.is_valid():	
        ...
        #return HttpResponse("<h2>Ваш пакет документов успешно собран</h2>")
        # return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        messages.success(request, 'Ваш документ успешно собран')
        return redirect('/')
    else:
...
def lin(request):
    ...
    return redirect('/')
        #return redirect('http://xn------5cdabbpnldcprfc4ag7bdhqdgn0ae1br.xn--p1ai:8080/feedback_page.php')
        #return HttpResponse("<h2>Ваш пакет документов успешно отправлен на {0}</h2>".format(email))
        #return render_to_response('first.html', message='Ваш документ успешно собран')
